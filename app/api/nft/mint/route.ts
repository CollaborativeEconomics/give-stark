  ...
  try {
    ...
    if (!txid) {
      return Response.json(
        { error: 'Required txid is missing' },
        { status: 400 }
      );
    }
    ...
    //   return Response.json(
    //     { error: 'Transaction not valid, wrong sender' },
    //     { status: 500 }
    //   );
    // }
    ...
    if (!initiative || initiative?.error) {
      ...
      return Response.json(
        { error: 'Initiative info not found' },
        { status: 500 }
      );
    }
    ...
    const ctr721 =
      '0x045d8da8227be85fec28cd4ba98b33bb1fba0408e1d78947a2d628c34e06ed4c';
