...
import { getDonationById, getStoryById } from '@/lib/utils/registry';
...
export default async function NFT(props: { params: { id: string } }) {
  ...
  let story = null;
  let title =
    'Your donation is waiting to be spent improving the world, check back later';
  if (donation.storyId) {
    story = await getStoryById(donation.storyId);
    title = "Your donation's impact on the world";
  }
  ...
  return (
    <main className="flex min-h-screen flex-col items-stretch container mt-12 pt-24">
              ...
              <div>
                <Link href={'/organizations/' + donation.organization.id}>
                  <OrganizationAvatar
                    ...
                    image={donation.organization.image}
                  />
                </Link>
              </div>
                ...
                {donation.initiative.category ? (
                  <>
                    <h1 className="text-sm">
                      {donation.initiative.category?.title}
                    </h1>
                    <Image
                      src={donation.initiative.category?.image}
                      width={96}
                      height={96}
                      alt="Category"
                    />
      ...
      <div>
        <h1 className="mt-24 mb-8 text-2xl text-center">{title}</h1>
