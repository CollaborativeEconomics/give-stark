...
import { getNFTById, getDonationById, getStoryById } from '@/lib/utils/registry'
...
export default async function NFT(props: {params:{id:string}}) {
  ...
  let story = null
  let title = "Your donation is waiting to be spent improving the world, check back later"
  if(nft.donationId){
    const donation = await getDonationById(nft.donationId)
    if(donation?.storyId){
